...
const Stations = () => {
  ...
  const [metricsSummary, setMetricsSummary] =
    useState<StationSummaryMetrics | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [layoutToggleMode, setLayoutToggleMode] = useState<"Table" | "Grid">(
    "Table",
  );
  const [formState, setFormState] = useState<NewStationFormState>(
    InitialStationFormPayload,
  );

  const [selectedStatusCardFilter, setSelectedStatusCardFilter] = useState<
    "All" | "Available" | "Charging" | "Offline" | "Maintenance"
  >("All");
  ...
  const executeFormSubmission = (e: React.FormEvent) => {
    e.preventDefault();
    console.log(
      "Transmitting finalized station entry package payload mapping to backend API:",
      formState,
    );
  ...
  const handleStatusCardClickToggle = (
    targetFilter: "Available" | "Charging" | "Offline" | "Maintenance",
  ) => {
    setSelectedStatusCardFilter((prev) =>
      prev === targetFilter ? "All" : targetFilter,
    );
  };

  const filteredItems = stationsList.filter((station) => {
    const matchesCardFilter =
      selectedStatusCardFilter === "All" ||
      station.status === selectedStatusCardFilter;
          ...
          <div className="stations-headline-title-stack">
            {!isCreateStationRoute ? (
              <>
                ...
                <p>
                  {metricsSummary?.totalAcrossNetwork || 0} stations across the
                  network
                </p>
              </>
            ) : (
              <>
                <div className="vendors-wizards-breadcrumb-navigation">
                  <span
                    className="vendors-breadcrumb-link"
          ...
          <>
            {metricsSummary && (
              <div className="stations-metric-summary-ribbon-grid">
                ...
                >
                  ...
                  <p className="summary-counter-value">
                    {metricsSummary.availableCount}
                  </p>
                </div>
                ...
                >
                  ...
                  <p className="summary-counter-value">
                    {metricsSummary.maintenanceCount}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="stations-filtering-controls-wrapper-bar">
              <div className="stations-input-search-lens-box-frame">
                ...
                <input
                  ...
                  placeholder={
                    selectedStatusCardFilter === "All"
                      ? "Search stations..."
                      : `Search ${selectedStatusCardFilter.toLowerCase()} stations...`
                  }
                  value={searchQuery}
              ...
              <div className="stations-layout-view-toggle-tabs-box">
                <button
                  type="button"
                  className={`stations-view-toggle-pill-btn ${layoutToggleMode === "Table" ? "active" : ""}`}
                  onClick={() => setLayoutToggleMode("Table")}
                >
                  Table
                </button>
                <button
                  type="button"
                  className={`stations-view-toggle-pill-btn ${layoutToggleMode === "Grid" ? "active" : ""}`}
                  onClick={() => setLayoutToggleMode("Grid")}
                >
                  Grid
                </button>
              </div>
                        ...
                        <tr key={station.id}>
                          <td>
                            <div className="station-table-identity-block">
                              <span
                                className={`station-table-dot-bullet-indicator ${station.status.toLowerCase()}`}
                              ></span>
                              <div className="vendor-meta-stacked-lines">
                                <span className="vendor-table-primary-headline-text">
                                  {station.name}
                                </span>
                                <span className="vendor-table-secondary-subtext-line">
                          ...
                          <td>
                            <span
                              className="vendor-table-primary-headline-text"
                          ...
                          <td>
                            <div className="station-table-utilization-bar-cell-container">
                              <div className="station-micro-linear-progress-bar-track">
                                <div
                                  className="station-micro-linear-progress-bar-fill-indicator"
                                  style={{
                                    width: `${station.utilizationPercentage}%`,
                                  }}
                                ></div>
                              </div>
                              <span
                                className="vendor-table-secondary-subtext-line"
                          ...
                          <td>
                            <span
                              className={`station-table-semantic-pill-status-box ${station.status.toLowerCase()}`}
                            >
                          ...
                          <td>
                            <div className="vendor-row-actions-group-cell">
                              <button
                                ...
                                aria-label="Open Map Trace Detail View"
                                title="View"
                              >
                                <Eye size={14} />
                              </button>
                              ...
                              <button
                                type="button"
                                className="vendor-row-action-icon-trigger-btn suspend"
                                aria-label="Toggle Power Lifecycle"
                                title="Disable"
                              >
                                <Power size={14} />
                              </button>
              ...
              <div className="stations-operational-mesh-grid-wrapper">
                {filteredItems.length === 0 ? (
                  ...
                  filteredItems.map((station) => (
                    <div
                      key={station.id}
                      className="station-grid-card-item-box"
                    >
                      <div className="station-grid-card-header-line">
                        <span
                          className={`station-code-pill-tag ${station.status.toLowerCase()}`}
                        >
                          <span className="grid-header-micro-dot">●</span>
                          <span>{station.stationCode}</span>
                        </span>
                        <span
                          className={`station-grid-status-txt ${station.status.toLowerCase()}`}
                        >
                          {station.status}
                        </span>
                      </div>
                      <h3>{station.name}</h3>
                      <p className="grid-card-location-subtext">
                        {station.location}
                      </p>
                      <p className="grid-card-vendor-label-title">
                        Vendor: <span>{station.vendor}</span>
                      </p>

                      <div className="station-grid-card-counters-flex-row">
                        <div className="counter-block-node">
                          <span
                            className="digit-bold"
                            style={{ color: "#24cb71" }}
                          >
                      ...
                      <div className="grid-card-chart-bar-footer-row">
                        <div
                          className="station-micro-linear-progress-bar-track"
                          style={{ height: "4px" }}
                        >
                          <div
                            className="station-micro-linear-progress-bar-fill-indicator"
                        ...
                        >
                          {station.utilizationPercentage}% busy
                        </span>
                      </div>
                    </div>
                  ))
            ...
            <form onSubmit={executeFormSubmission}>
              <div className="station-form-input-fields-split-grid">
                <div className="station-form-input-block-node">
                  ...
                  <input
                    type="text"
                    className="station-form-text-input-field"
                    placeholder="e.g., BKC EV Plaza"
                    value={formState.stationName}
                    onChange={(e) =>
                      handleFieldMutation("stationName", e.target.value)
                    }
                  />
                </div>
                <div className="station-form-input-block-node">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFieldMutation("stationCode", e.target.value)
                    }
                  />
                </div>
                <div className="station-form-input-block-node">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFieldMutation("contactNumber", e.target.value)
                    }
                  />
                </div>
                <div className="station-form-input-block-node">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFieldMutation("workingHours", e.target.value)
                    }
                  />
                </div>
                <div className="station-form-input-block-node">
                  <label>Vendor</label>
                  <select
                    className="station-form-dropdown-select-control"
                    value={formState.vendor}
                    onChange={(e) =>
                      handleFieldMutation("vendor", e.target.value)
                    }
                  >
                    <option value="GreenCharge India">GreenCharge India</option>
                    <option value="ChargePoint Networks">
                ...
                <div className="station-form-input-block-node">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleFieldMutation("state", e.target.value)
                    }
                  >
                ...
                <div className="station-form-input-block-node full-width-span-row">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFieldMutation("fullAddress", e.target.value)
                    }
                  />
                </div>
                <div className="station-form-input-block-node">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFieldMutation("latitude", e.target.value)
                    }
                  />
                </div>
                <div className="station-form-input-block-node">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFieldMutation("longitude", e.target.value)
                    }
                  />
                </div>
              </div>
              ...
              <div
                className="station-form-input-block-node full-width-span-row"
              ...
              >
                ...
                <div className="station-form-interactive-map-integration-viewport">
                  ...
                  <p style={{ fontSize: "11px", opacity: 0.7 }}>
                    Lat: {formState.latitude || "19.0596"}, Lng:{" "}
                    {formState.longitude || "72.8656"}
                  </p>
                </div>
              </div>
                ...
                <div className="station-form-amenities-checkbox-flex-matrix">
                  {targetAmenitiesList.map((amenity) => {
                    ...
                    return (
                      <label
                        key={amenity}
                        className="amenity-checkbox-custom-label-pill"
                      >
                        <input
                          type="checkbox"
                          checked={isChecked}
                          onChange={() =>
                            handleAmenitiesCheckboxToggle(amenity)
                          }
                        />
                        <span>{amenity}</span>
                      </label>
